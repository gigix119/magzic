        ...
        <>
          ...
          <div className="md:hidden" style={{ borderBottom: '1px solid var(--border)' }}>
            {poz.map(p => {
              ...
              const mainPrice  = isGross ? grossPrice : netPrice
              const mainSum    = isGross ? sumaBrutto : sumaNetto
              const altSum     = isGross ? sumaNetto  : sumaBrutto
              const jednostka  = p.jednostka || (p.towary?.nazwa?.length >= 2 ? p.towary?.jednostka : null) || ''

              return (
                <div key={p.id} className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      {displayNazwa ? (
                        <p className="text-sm font-medium break-words" style={{ color: 'var(--text)', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: 1.35 }}>
                          {displayNazwa}
                        </p>
                      ) : (
                        <p className="text-sm italic" style={{ color: 'var(--muted)' }}>(brak towaru)</p>
                      )}
                      {productName && (p.raw_name || p.rawName) && productName !== (p.raw_name || p.rawName) && (
                        <p className="text-xs mt-0.5 truncate" style={{ color: 'var(--text-2)' }}>PDF: {p.raw_name || p.rawName}</p>
                      )}
                      {(p.indeks || p.sku) && (
                        <p className="text-xs mt-0.5" style={{ color: 'var(--muted)', fontFamily: 'DM Mono, monospace' }}>{p.indeks || p.sku}</p>
                      )}
                      <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                        {isService && (
                          <span className="text-xs font-medium px-1.5 py-0.5 rounded" style={{ background: '#f0f9ff', color: '#0369a1', border: '1px solid #bae6fd' }}>Usługa</span>
                        )}
                        <span className="text-xs font-medium px-1.5 py-0.5 rounded-full" style={{ background: badge.bg, color: badge.color, border: `1px solid ${badge.border}` }}>
                          {badge.label}
                        </span>
                        <span className="text-xs" style={{ color: 'var(--muted)' }}>VAT {vatRate}%</span>
                      </div>
                    </div>
                    <div className="flex-shrink-0 text-right">
                      <p className="text-xs" style={{ color: 'var(--muted)' }}>
                        {Number(p.ilosc)} {jednostka} × {mainPrice.toFixed(2)} zł
                      </p>
                      <p className="font-bold mt-0.5" style={{ fontFamily: 'DM Mono, monospace', color: isGross ? '#1d4ed8' : '#166534', fontSize: 15 }}>
                        {mainSum.toFixed(2)} zł
                      </p>
                      {altSum > 0 && Math.abs(altSum - mainSum) > 0.005 && (
                        <p className="text-xs" style={{ color: 'var(--muted)', fontFamily: 'DM Mono, monospace' }}>
                          {isGross ? 'netto:' : 'brutto:'} {altSum.toFixed(2)} zł
                        </p>
                      )}
                    </div>
                  </div>
                  {fak.status === 'robocza' && (
                    <div className="flex gap-2 mt-2.5 justify-end">
                      <button onClick={() => onEditPoz(p, fak)} className="flex items-center gap-1.5 px-3 rounded-lg text-xs font-medium" style={{ background: 'var(--table-sub)', color: 'var(--text-2)', border: '1px solid var(--border)', minHeight: 36 }}>
                        <Pencil size={11} /> Edytuj
                      </button>
                      <button onClick={() => onDeletePoz(p)} className="flex items-center gap-1.5 px-3 rounded-lg text-xs font-medium" style={{ background: 'rgba(239,68,68,0.08)', color: '#dc2626', border: '1px solid rgba(239,68,68,0.3)', minHeight: 36 }}>
                        <Trash2 size={11} /> Usuń
                      </button>
                    </div>
                  )}
            ...
            {/* Mobile total */}
            <div className="px-4 py-3 flex items-center justify-between" style={{ background: 'var(--table-sub)', borderTop: '2px solid var(--border)' }}>
              <span className="text-sm font-semibold" style={{ color: 'var(--text-2)' }}>
                {isGross ? 'Razem brutto:' : 'Razem netto:'}
              </span>
              <div className="text-right">
                <p className="font-bold" style={{ fontFamily: 'DM Mono, monospace', color: isGross ? '#1d4ed8' : '#166534', fontSize: 16 }}>
                  {mainTotal.toFixed(2)} zł
                </p>
                {Math.abs(altTotal - mainTotal) > 0.005 && (
                  <p className="text-xs" style={{ color: 'var(--muted)', fontFamily: 'DM Mono, monospace' }}>
