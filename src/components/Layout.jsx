    ...
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg)' }}>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2" aria-label="Nawigacja główna">
          ...
          <SidebarNavLink to="/zlecenia" icon={zlecenieItem.icon} label={zlecenieItem.label} onClick={close} alertCount={alertCount} />
          <SidebarNavLink to="/alerty" icon={Bell} label="Alerty" showBadge onClick={close} alertCount={alertCount} />

          {/* STANDARDY — only for cleaning/hospitality */}
          {hasPakiety && (
            <>
              <div style={{ ...GROUP_LABEL_STYLE, marginTop: 4 }}>Standardy</div>
              <SidebarNavLink to="/pakiety" icon={Sparkles} label="Pakiety sprzątania" onClick={close} alertCount={alertCount} />
            </>
          )}
          ...
          {/* BACKEND — owner only, visually separated */}
          {ownerAccess && (
            <>
