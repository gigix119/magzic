...
function SupplierComparisonResult({ comparison, text }) {
...
const PRICE_HISTORY_COLS = [
  { key: 'date',        label: 'Data',      align: 'right', mono: true },
...
function ProductPriceHistoryResult({ history, text }) {
...
export default function AssistantResult({ intent, text, structuredData }) {
  ...
  if (structuredData && intent === 'product_price_history') {
    return <ProductPriceHistoryResult history={structuredData} text={text} />
  }

  if (structuredData && intent === 'compare_suppliers') {
    return <SupplierComparisonResult comparison={structuredData} text={text} />
  }
