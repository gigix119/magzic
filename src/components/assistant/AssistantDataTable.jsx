...
export default function AssistantDataTable({ title, columns, rows, emptyMessage, exportable, exportFilename, exportLabel }) {
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
      ...
      >
        <div className="text-sm font-semibold min-w-0" style={{ color: 'var(--text)', fontSize: 13 }}>{title}</div>
        {exportable && rows?.length > 0 && (
          <button
            onClick={() => downloadCsv(exportFilename ?? 'export.csv', rows, columns)}
            className="flex-shrink-0 flex items-center gap-1 rounded-md px-2 py-0.5 transition-opacity hover:opacity-80 active:opacity-60"
            style={{
            ...
            <thead>
              <tr style={{ background: 'var(--table-sub)' }}>
                {columns.map(col => (
                  <th
                    key={col.key}
                    className={`py-2.5 font-medium ${col.align === 'right' ? 'text-right pr-4 pl-2' : 'text-left px-4'} ${col.hideOnMobile ? 'hidden sm:table-cell' : ''}`}
              ...
              {rows.map((row, idx) => (
                ...
                >
                  {columns.map(col => (
                    <td
                      key={col.key}
                      className={`py-3 ${col.align === 'right' ? 'text-right pr-4 pl-2' : 'px-4'} ${col.hideOnMobile ? 'hidden sm:table-cell' : ''}`}
                      style={{
