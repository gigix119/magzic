...
export default function AlertyTab() {
  ...
  async function fetchPriceAlerts() {
    ...
    try {
      ...
      if (error) {
        if (error.code === '42P01') { setPriceAlerts([]); return }
        throw error
      }
        ...
        <div className="rounded-xl overflow-hidden mb-6" style={{ border: '1px solid var(--border)' }}>
          ...
          {!showAllInsights && insightsAll.length > 7 && (
            <div className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
              <button onClick={() => setShowAllInsights(true)} className="w-full py-2.5 text-sm font-medium rounded-xl border transition-colors" style={{ color: '#2563eb', background: 'rgba(59,130,246,0.04)', borderColor: 'rgba(59,130,246,0.2)', minHeight: 44 }}>
          ...
          {showAllInsights && insightsAll.length > 7 && (
            <div className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
              <button onClick={() => setShowAllInsights(false)} className="w-full py-2.5 text-sm font-medium rounded-xl border transition-colors" style={{ color: 'var(--muted)', background: 'var(--table-sub)', borderColor: 'var(--border)', minHeight: 44 }}>
                Zwiń ↑
              </button>
            </div>
          )}
        ...
        {allAlerts.length === 0 ? (
          ...
          <>
              ...
              {(showAllStockAlerts ? allAlerts : allAlerts.slice(0, 7)).map(a => {
                ...
                return (
                  <div key={a.id} className="flex items-start gap-3 px-4 py-3" style={{ background: cfg.bg }}>
                    <span style={{ color: cfg.color, flexShrink: 0, marginTop: 2 }}><AlertIcon type={a.type} size={14} /></span>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium break-words" style={{ color: 'var(--text)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{a.towar.nazwa}</p>
                      <p className="text-xs mt-0.5" style={{ color: 'var(--text-2)' }}>{alertMessage(a)}</p>
                    </div>
                  ...
                  {(showAllStockAlerts ? allAlerts : allAlerts.slice(0, 7)).map((a, idx) => {
                    ...
                    return (
                      <tr key={a.id} className="table-row" style={{ background: idx % 2 === 0 ? 'var(--table-even)' : 'var(--table-odd)', borderTop: '1px solid var(--border)' }}>
                        ...
                        <td className="px-5 py-3 text-center"><Badge variant={cfg.badgeVariant}>{cfg.label}</Badge></td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            {!showAllStockAlerts && allAlerts.length > 7 && (
              <div className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
                <button onClick={() => setShowAllStockAlerts(true)} className="w-full py-2.5 text-sm font-medium rounded-xl border transition-colors" style={{ color: '#2563eb', background: 'rgba(59,130,246,0.04)', borderColor: 'rgba(59,130,246,0.2)', minHeight: 44 }}>
            ...
            {showAllStockAlerts && allAlerts.length > 7 && (
              <div className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
                <button onClick={() => setShowAllStockAlerts(false)} className="w-full py-2.5 text-sm font-medium rounded-xl border transition-colors" style={{ color: 'var(--muted)', background: 'var(--table-sub)', borderColor: 'var(--border)', minHeight: 44 }}>
                  Zwiń ↑
                </button>
              </div>
            )}
              ...
              <tbody>
                {alertyCenowe.map((alert, idx) => (
                  <tr key={alert.id} className="table-row" style={{ background: idx % 2 === 0 ? 'var(--table-even)' : 'var(--table-odd)', borderTop: '1px solid var(--border)' }}>
                    ...
                    <td className="px-5 py-3 text-center"><Badge variant={alert.aktywny ? 'green' : 'zinc'}>{alert.aktywny ? 'Aktywny' : 'Wyłączony'}</Badge></td>
      ...
      <div className="rounded-xl overflow-hidden mt-6" style={{ border: '1px solid var(--border)' }}>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 px-5 py-3" style={{ background: 'var(--table-head)', borderBottom: '1px solid var(--border)' }}>
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <TrendingUp size={16} style={{ color: 'var(--c-action)', flexShrink: 0 }} />
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {priceAlerts.length > 1 && (
              <button onClick={markAllPriceAlertsRead} className="flex items-center gap-1.5 text-xs rounded-lg px-3 py-1.5 font-medium text-white" style={{ background: '#22c55e', minHeight: 36 }}>
                <CheckCheck size={12} /> Wszystkie przeczytane
              </button>
            )}
            <button onClick={fetchPriceAlerts} disabled={priceAlertsLoading} className="text-xs rounded-lg px-3 py-1.5 font-medium" style={{ background: 'var(--table-sub)', color: 'var(--text-2)', border: '1px solid var(--border)', minHeight: 36 }}>
        ...
        {priceAlertsLoading ? (
          <div className="text-center py-8" style={{ background: 'var(--table-even)', color: 'var(--muted)', fontSize: 13 }}>Ładuję alerty…</div>
        ) : priceAlerts.length === 0 ? (
          <div className="text-center py-10" style={{ background: 'var(--table-even)', color: 'var(--muted)' }}>
            <p className="text-sm">Brak nieprzeczytanych anomalii cenowych</p>
            <p className="text-xs mt-1" style={{ color: 'var(--muted)' }}>Alerty są generowane automatycznie przy zatwierdzaniu faktur</p>
          </div>
        ) : (
          <>
            <div className="md:hidden divide-y" style={{ borderColor: 'var(--border)' }}>
                ...
                return (
                  <div key={alert.id} className="px-4 py-3" style={{ background: cfg.bg }}>
                    <div className="flex items-start gap-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium break-words" style={{ color: 'var(--text)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{alert.towary?.nazwa || '—'}</p>
                        ...
                        {(alert.faktury?.numer || alert.kontrahenci?.nazwa) && (
                          <p className="text-xs mt-0.5 truncate" style={{ color: 'var(--text-2)' }}>{[alert.faktury?.numer, alert.kontrahenci?.nazwa].filter(Boolean).join(' · ')}</p>
                        )}
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          <Badge variant={cfg.badgeVariant}>{cfg.label}</Badge>
                          {alert.cena_aktualna != null && <span className="text-xs" style={{ fontFamily: 'DM Mono, monospace', color: 'var(--text-2)' }}>{Number(alert.cena_aktualna).toFixed(2)} zł</span>}
                          {alert.roznica_procent != null && <span className="text-xs font-semibold" style={{ fontFamily: 'DM Mono, monospace', color: alert.roznica_procent > 0 ? '#dc2626' : '#16a34a' }}>{alert.roznica_procent > 0 ? '+' : ''}{Number(alert.roznica_procent).toFixed(1)}%</span>}
                        </div>
                      </div>
                      <button onClick={() => markPriceAlertRead(alert.id)} className="flex-shrink-0 flex items-center justify-center rounded-lg" style={{ color: '#22c55e', minWidth: 44, minHeight: 44 }} title="Oznacz jako przeczytane"><CheckCheck size={14} /></button>
                    </div>
                  </div>
                )
              })}
                  ...
                  {(showAllPriceAnomalies ? priceAlerts : priceAlerts.slice(0, 7)).map((alert, idx) => {
                    ...
                    return (
                      <tr key={alert.id} className="table-row" style={{ background: idx % 2 === 0 ? 'var(--table-even)' : 'var(--table-odd)', borderTop: '1px solid var(--border)' }}>
                        ...
                        <td className="px-4 py-3 text-right text-xs hidden md:table-cell" style={{ fontFamily: 'DM Mono, monospace', color: 'var(--text-2)' }}>{alert.cena_aktualna != null ? `${Number(alert.cena_aktualna).toFixed(2)} zł` : '—'}</td>
                        <td className="px-4 py-3 text-right text-xs hidden md:table-cell" style={{ fontFamily: 'DM Mono, monospace', color: alert.roznica_procent > 0 ? '#dc2626' : '#16a34a' }}>{alert.roznica_procent != null ? `${Number(alert.roznica_procent).toFixed(1)}%` : '—'}</td>
                        <td className="px-4 py-3 text-center"><Badge variant={cfg.badgeVariant}>{cfg.label}</Badge></td>
                        <td className="px-3 py-3 text-center"><button onClick={() => markPriceAlertRead(alert.id)} className="p-1.5 rounded-lg" style={{ color: '#22c55e' }} title="Oznacz jako przeczytane"><CheckCheck size={13} /></button></td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            {!showAllPriceAnomalies && priceAlerts.length > 7 && (
              <div className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
                <button onClick={() => setShowAllPriceAnomalies(true)} className="w-full py-2.5 text-sm font-medium rounded-xl border transition-colors" style={{ color: '#2563eb', background: 'rgba(59,130,246,0.04)', borderColor: 'rgba(59,130,246,0.2)', minHeight: 44 }}>
            ...
            {showAllPriceAnomalies && priceAlerts.length > 7 && (
              <div className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
                <button onClick={() => setShowAllPriceAnomalies(false)} className="w-full py-2.5 text-sm font-medium rounded-xl border transition-colors" style={{ color: 'var(--muted)', background: 'var(--table-sub)', borderColor: 'var(--border)', minHeight: 44 }}>
                  Zwiń ↑
                </button>
              </div>
            )}
      ...
      {showModal && (
        <Modal title="Nowy alert cenowy" onClose={() => setShowModal(false)}>
          <form onSubmit={handleSaveAlert} className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="al_a" checked={form.aktywny} onChange={e => setForm(f => ({ ...f, aktywny: e.target.checked }))} style={{ accentColor: 'var(--c-action)' }} />
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="submit" disabled={saving} className="flex-1 rounded-lg py-2 text-sm font-medium text-white" style={{ background: 'var(--c-action)', opacity: saving ? 0.7 : 1 }}>
