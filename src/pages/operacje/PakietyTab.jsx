...
export default function PakietyTab() {
      ...
      <div className="space-y-3">
        {pakiety.length === 0 ? (
          <div className="text-center py-16 rounded-xl" style={{ background: 'var(--card)', border: '1px solid var(--border)', color: 'var(--muted)' }}>
            ...
            <p className="font-medium mb-1" style={{ color: 'var(--text)' }}>Brak pakietów</p>
            <p className="text-sm" style={{ color: 'var(--muted)' }}>Utwórz pakiet sprzątania z listą środków i materiałów.</p>
          </div>
        ) : (
          pakiety.map(pak => {
            const items = elementy[pak.id] || []
                ...
                <div className="md:hidden px-4 py-4" style={{ background: isOpen ? 'var(--table-odd)' : 'transparent' }}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0" onClick={() => setExpanded(isOpen ? null : pak.id)} style={{ cursor: 'pointer' }}>
                      ...
                      {pak.opis && <p className="text-xs mt-0.5" style={{ color: 'var(--text-2)', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{pak.opis}</p>}
                    </div>
            ...
            {execBraki.length > 0 && (
              <div className="rounded-lg p-3 flex gap-2" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.3)' }}>
                ...
                <div className="text-sm" style={{ color: '#ef4444' }}>
                  ...
                  {execBraki.map((b, i) => <p key={i}>{b.nazwa}: potrzeba {b.potrzebne}, dostępne {b.dostepne}</p>)}
                </div>
              </div>
            )}
